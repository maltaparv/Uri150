...
namespace Uri150
{
    public partial class Form1 : Form
    {
        ...
        {
            #region --- описание получаемых данных и уже третий по счёту алгоритм парсинга
            ...
            26 <03h>
                ...
                Из практического опыта известно, что в переданных данных должны присутствовать 11 результатов измерения
                ...
                Если есть нужный анализ из 11-ти - формируем строку SQL, дописывая выделенные данные в конец.
                Тогда (предположительно) даже для других типов тест-полосок, на которых есть дополнительные анализы:
                - "CR", "MA", "Ca", "ACR" - этот алгоритм должен работать! (Не проверял, т.к. нет тест-полосок типа 14G и других)
            */
            #endregion --- описание получаемых данных и алгоритм парсинга
            ...
            //01NO.000003  - это порядковый номер теста, присваиваемый анализатором, увеличивающийся на 1 при следующем пациенте.
            n = 1; s = Line[n]; 
            ...
            nHistNo = cHistNo.Length == 0 ? (0) : (Convert.ToInt64(cHistNo)); // 13 цифр для ввода ID на анализаторе не влезают в int32!
            if (nHistNo > MaxHistNo)
            {
                string err1 = $"Введённый номер истории {nHistNo} больше максимально допустимого {MaxHistNo}!"; // MaxHistNo в ini-файле
                Add_RTB(RTBout, $"\n\n{err1}", Color.Red);
                nHistNo = 0;
                WLog(err1);
            ...
            if (nHistNo == 0)
            {
                string err1 = $"Не введён номер истории! No: {testNo}.";  // дата: {dateDone} ещё не выделена!
